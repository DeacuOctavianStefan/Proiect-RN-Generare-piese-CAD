...
```
Am ales modelul de la exp 2 deoarece ofera cel mai bun raport intre timpul de antrenare si acuratete, fara a face sacrificii. Pentru a se verifica datele de mai sus, sa se ruleze eval_accuracy.py pentru modelul dorit.
...
|----------------|-------------------|------------------------|-----------------|
| **Model încărcat** | `json_sft_distilgpt2_p3200` | `optimized_model` | +11.42% accuracy |
...
1. **Model înlocuit:** `models/trained_model.h5` → `models/optimized_model.h5`
   - Îmbunătățire: Accuracy +11.48%, F1 +2%
   - Motivație: Raport intre timp de antrenare si perfomants cel mai bun
