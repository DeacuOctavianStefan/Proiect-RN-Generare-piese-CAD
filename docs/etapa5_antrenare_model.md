...
|--------------------|-------------------|-----------------|
| model_name | "distil-gpt2" | Baseline mic, usor de facut fine tuning si resurse scazute de operare |
| max_length | 196 | Limiteaza contextul pentru uz stabil de memorie; 192 este suficient pentru scopul actual al proiectului de genera un json scurt|
| epochs | 1 | Valideaza pipeline-ul  |
| lr | 5e-5 | Valoare comuna pentru GPT-2, echilibreaza invatatul cu stabilitatea modelului de baza |
| train_batch_size | 1 | Mentine memoria GPU folosita la un nivel scazut, evita spike-uri VRAM |
| eval_batch_size | 1 (default) | Evaluarea este step-based |
| max_steps | -1 (disabled) | Dorim antrenarea pe baza unui det intreg, trebuie sa fie epoch-based |
| use_lora | 0 | Nu este necesar pentru baseline |
| eval_abs_tol | 0.5 | Vom lasa o marja de eroare pentru a putea obtine o evaluare de accuracy corespunzatoare |

**Justificare detaliată epochs:**
```
Am ales numarul initial de epoci 1 pentru a crea un baseline de la care se poate verifica functionalitatea programului. De asemenea, pentru antrenarea initiala nu este necesar mai mult de o epoca. Numarul de epoci se poate modifica pentru a face fine tuning.
```

**Justificare detaliată train_batch_size:**
```
Am ales train_batch_size 1 datorita restrictiilor locale de hardware (NVIDIA Quadro P3200 GPU), avand la dispozitie decat 6GB VRAM. Un batch mai mare ar rezulta intr-un timp de antrenare mai crescut si nu are relevanta pentru un baseline training
```
...
**Completați pentru proiectul vostru:**
```
Modelul are o performanta slaba atunci cand nu ii sunt oferite suficiente date. Acesta poate genera piese eronate, etc.
```
